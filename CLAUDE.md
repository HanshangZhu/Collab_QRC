...
## Active state (2026-05-25) — CUDA-MPPI PathFollow-carrot bug fixed (creep → cruise) + Go2W gait-switch speed cap

The Go2W CUDA-MPPI controller was creeping at a constant **~0.10 m/s** on open, obstacle-free stretches despite `vx_max=0.50` — the robot stayed in leg gait because the wheel-engage gate (needs sustained forward speed) was never satisfied. Root-caused to a bug in the vendored CUDA backend (NOT the costmap, which probed 94 % free locally), fixed it, then capped speed to stay clear of the high-momentum gait-switch tip zone. Diagnosed live with rclpy probes (the ros2 CLI is wedged in this env): `vx mean=0.097 max=0.114` flat, never exceeding 0.114, with a **9 m** planned path to a goal **7 m** away — so neither goal-decel nor clutter.

### The bug — `furthest_reached_path_point` computed wrong (PathFollow carrot pinned to the robot)

In Nav2 MPPI, forward speed is driven almost entirely by **PathFollowCritic's carrot** = `furthest_reached_path_point + offset_from_furthest`. The carrot is meant to *advance ahead of the robot as the trajectory bundle reaches further down the path* — a feedback loop that grows speed up to `vx_max`. [`cuda_backend.cu`](src/vendor/nav_algo_ros1/nav_algo_mppi_cuda/src/cuda_backend.cu) computed `furthest_reached` as **the path index closest to the robot's *current* pose** — a single point, computed once per cycle *before* the integrate kernel ran (so trajectories weren't even available yet; that's *why* they fell back to closest-to-robot). That pins the carrot at `robot + offset_from_furthest(6)` ≈ **0.30 m ahead forever**, killing the feedback loop. MPPI then optimizes its 2.8 s horizon (56 × 0.05) to land the trajectory END on that fixed carrot → steady-state `vx = 0.30 m / 2.8 s ≈ 0.107 m/s`, matching the observed 0.097–0.114 exactly. Independent of path length, costmap, and the wheel threshold (which is why lowering the wheel gate barely helped).

### The fix — faithful port of `utils::findPathFurthestReachedPoint`, computed after integrate

Moved `furthest_reached` (and the PathAlign / PathAngle gates that depend on it) **inside the iteration loop, right after the integrate kernel**, and compute it as Nav2 does: for each of the B=2000 trajectories, the path index closest to that trajectory's **ENDPOINT**, then the **MAX** index across the bundle. `cudaMemcpy2D` pulls the last-timestep column (one float per trajectory, stride T) D2H; the B×P closest-point loop mirrors Nav2's CPU xtensor reduction. Restores the carrot-advances-with-speed feedback loop. Rebuilt `nav2_mppi_controller_cuda_plugin` (compiles the `.cu` directly). Adds ~0.5–1 ms/cycle (one D2H sync + host B×P) — negligible vs the 50 ms budget at 20 Hz.

**Mode-independent:** 2d (octomap) and 3d (elevation-mapping) creep identically on flat ground. 3d only *looked* fine because on the ramp the router's pitch-triggered force-wheel override floors `vx` to `pitch_climb_vx_mps=0.45` and bypasses MPPI's commanded speed entirely — masking the creep on the one slope.

### Speed cap to stay below the lethal gait-switch zone

With the carrot fixed, MPPI now actually reaches `vx_max`. Switching wheel↔legged at high momentum (~0.5–0.6 m/s) tips the robot, so [`nav2_go2w_full_stack.yaml`](src/go2w/go2w_config/config/nav/nav2_go2w_full_stack.yaml) `vx_max: 0.50 → 0.40` keeps cruise a clean margin below that band (switches always ≤ 0.40). The CUDA backend reads `vx_max` from `settings.constraints` ([cuda_backend.cu:449](src/vendor/nav_algo_ros1/nav_algo_mppi_cuda/src/cuda_backend.cu#L449)) so the cap propagates with no rebuild. Net: ~4× the old creep, safe switches.

### Wheel-engage made more readily (shared [`go2w_hybrid_motion.yaml`](src/go2w/go2w_config/config/control/go2w_hybrid_motion.yaml))

`wheel_linear_threshold: 0.18 → 0.10` and `wheel_engage_sustain_sec: 0.5 → 0.25` (was the router default) so wheel mode engages on moderate forward motion, not just brisk highways. Turning gates (`wheel_angular_threshold 0.30`, `wheel_curvature_threshold 0.45`) left unchanged so turns still route to legged — preserves the no-skid-steer-pivot guard.
...
### 2026-05-21 HIL / real-run operator updates

- **`lidar_range` now means radial point range, not height.** `scripts/real/real_run_monitor.sh` forwards `lidar_range=N` to `scripts/real/onboard_autonomy_noetic.sh`, which patches the active Point-LIO / FAST-LIO `mid360.yaml` `mapping.det_range=N`. This is a spherical/polar point radius cutoff in the SLAM preprocess/mapping layer. It does **not** change the Livox driver raw UDP output and it does **not** change the traversability ceiling.
- **Height filtering is separate.** Use `trav_height_range=N` for the elevation/traversability gate. The onboard launcher patches `trav_pipeline_ros1/config/elevation_mapping_go2w.yaml` `max_height_range` and `ramped_height_range_b`.
- **Laptop RViz2 is now bidirectional.** `scripts/launch/nx_viz_laptop.sh` runs ROS 2 UDP RX for NX viz topics and ROS 2 UDP TX for RViz `/goal_pose` → NX `/robot/move_base_simple/goal`. Manual mode is `explore=false`; the robot should still receive 2D Goal Pose via the relay.
- **RViz2 planned path relay.** ROS 1 NX `hil_relay_tx_node` forwards `/<ns>/move_base/SmacLatticePlannerROS/plan` (`nav_msgs/Path`) over UDP `plan_port=9008`; ROS 2 laptop `hil_relay_rx_node` republishes `/robot/planned_path`. `hil_nx.rviz` displays this as `Plan Path`.
- **RViz2 visual style.** `src/go2w/go2_gazebo_sim/rviz/hil_nx.rviz` is black/white/gray for report-friendly HIL viewing: black background, gray traversability cloud/grid, white odometry/path/frontier marker, gray manual goal marker.
- **CycloneDDS interface helper.** Real-run laptop execution must bind DDS to the currently active `192.168.123.x` NIC, not stale `enxc8a36240a4c7`.
  - [`scripts/real/laptop_ethernet_setup.sh`](scripts/real/laptop_ethernet_setup.sh): owns `GO2W_ETH_IFACE` auto-detect, stale `CYCLONEDDS_URI` cleanup, `connect_ethernet.sh` sourcing, and DDS interface sanity check.
  - [`scripts/real/real_run_monitor.sh`](scripts/real/real_run_monitor.sh): orchestration only; sources `LAPTOP_ETHERNET_SETUP_SCRIPT` and calls `setup_real_monitor_laptop_ethernet`.
  - [`scripts/test/test_real_monitor_laptop_ethernet_decoupling.sh`](scripts/test/test_real_monitor_laptop_ethernet_decoupling.sh): regression guard preventing Ethernet setup logic from drifting back into the monitor.
- **Real execution is not CHAMP.** Sim CHAMP drives a simulated joint-level/CHAMP controller. The real-run path is `move_base` MPPI → `/robot/cmd_vel` → laptop ROS 2 `cmd_vel_to_sport_bridge.py` → Unitree Sport API `/api/sport/request` with obstacle avoidance off. Low-speed real Go2 walking can look like torso yaw with weak translation if `linear.x` is below gait response/deadband or if the Sport API subscriber/control mode is not active. Debug this by comparing `/robot/cmd_vel`, `/api/sport/request`, and `/tmp/laptop_cmd_vel_to_sport.log` before retuning the planner.
...
25. **MPPI forward speed is the PathFollow carrot; `furthest_reached_path_point` must track the trajectory BUNDLE, not the robot (2026-05-25).** In Nav2 MPPI there is no explicit "go fast" reward — cruise speed emerges from PathFollowCritic chasing a carrot at `furthest_reached_path_point + offset_from_furthest`, where `furthest_reached` = MAX over all sampled trajectories of each trajectory's-ENDPOINT-closest path index (`utils::findPathFurthestReachedPoint`). That creates a feedback loop: faster trajectories reach further → carrot advances → speed grows to `vx_max`. If you instead compute `furthest_reached` as the path index closest to the robot's *current* pose (a tempting simplification, e.g. when porting and the integrated trajectories aren't available yet), the carrot pins at a fixed `offset_from_furthest` cells ahead and MPPI converges to `vx ≈ carrot_dist / (T·model_dt)` — a constant creep (~0.1 m/s for our 6-cell offset + 2.8 s horizon) **independent of path length, costmap occupancy, and any wheel/speed thresholds downstream**. Symptom: steady low `vx` with a hard ceiling, on a clean costmap with a long path. The CUDA backend must therefore compute `furthest_reached` AFTER the integrate kernel (from the trajectory endpoints), not before. Reference: [`cuda_backend.cu`](src/vendor/nav_algo_ros1/nav_algo_mppi_cuda/src/cuda_backend.cu) block "1b".
