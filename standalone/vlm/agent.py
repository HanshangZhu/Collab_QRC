...
def run_direct(
    *,
    ...
    """Single-shot LLM call WITHOUT LangGraph tool binding.

    Used for providers where create_react_agent's tool-calling round-trip is
    unreliable (e.g. Groq llama-4-scout returns HTTP 400 'Failed to call a
    function' on the OpenAI-compat /chat/completions endpoint).

    The VLM still gets the full vision+text input and is expected to emit the
    standard JSON contract {goal, reason, artifact_seen, artifact_pos, done}.
    Without tools, `log_artifact` cannot be invoked from the agent; the caller
    should synthesize an artifact-log entry from the returned dict when
    artifact_seen is True.
    """
    llm = build_llm(provider, model, temperature=temperature, max_tokens=max_tokens)
    ...
    final_text = resp.content if isinstance(resp.content, str) else ""
    ...
    # One retry if model returned non-JSON garbage.
    if "parse_error" in parsed.get("reason", ""):
        import sys
        print(f"[VLM direct] parse_error — raw final_text was: {final_text[:120]!r}",
              file=sys.stderr)
        try:
            retry_msgs = messages + [
            ...
            retry_text = (
                retry_resp.content if isinstance(retry_resp.content, str) else ""
            )
            if isinstance(retry_resp.content, list):
            ...
            print(f"[VLM direct] retry exception: {exc}", file=sys.stderr)
