...
Image 2 — ROBOT FRONT CAMERA (RGB, if provided):
  ...
  * Scan it for colored objects: spheres, boxes, cylinders, unusual items.
  * If you spot one, call log_artifact(x, y, description) ONCE to record it.
  * Estimate world (x, y) from robot pose + bearing to the object.
...
def build_user_prompt(scene: Dict[str, Any], max_chars: int = 4000) -> str:
    ...
    best_hint = (
        f"Highest info_gain candidate: ({best['x']}, {best['y']}) "
        f"info_gain={best['info_gain']:.1f} m²"
        if best else "No frontier candidates — output done: true."
    )
    ...
    return (
        ...
        "2. ARTIFACT: if Image 2 (camera) shows a colored object/marker, call "
        "log_artifact(x, y, description) once.\n"
