...
def _mock_pick_goal(scene: dict, strategy: str = "mock") -> str:
    """Picks a goal from scene's frontier_candidates list.

    Strategies
    ----------
    mock / info_gain
        Pick the frontier with the highest ``info_gain`` score (most unknown
        area reachable from that candidate). This is the classic information-
        gain heuristic used by greedy frontier planners (Yamauchi 1997).
    random
        Pick uniformly at random from the A*-validated candidates.
        Useful as a lower-bound baseline.
    greedy_nearest
        Pick the candidate with the smallest Euclidean distance to the robot's
        current pose — equivalent to "nearest unexplored frontier" in
        Yamauchi's original formulation (without the info-gain weighting).
    """
    import random as _random
    cands = scene.get("frontier_candidates", [])
    if not cands:
        return json.dumps({"goal": None, "reason": f"{strategy}: no frontiers"})

    s = (strategy or "mock").lower()

    if s == "random":
        chosen = _random.choice(cands)
        return json.dumps({
            "goal": {"x": float(chosen["x"]), "y": float(chosen["y"])},
            "reason": f"random: picked uniformly from {len(cands)} candidates",
        })

    if s in ("greedy_nearest", "greedy"):
        robot = scene.get("robot", {})
        rx, ry = float(robot.get("x", 0.0)), float(robot.get("y", 0.0))
        chosen = min(cands, key=lambda c: (c["x"] - rx) ** 2 + (c["y"] - ry) ** 2)
        dist = ((chosen["x"] - rx) ** 2 + (chosen["y"] - ry) ** 2) ** 0.5
        return json.dumps({
            "goal": {"x": float(chosen["x"]), "y": float(chosen["y"])},
            "reason": f"greedy_nearest: closest frontier at {dist:.2f} m",
        })

    # Default: highest info_gain (mock / info_gain)
