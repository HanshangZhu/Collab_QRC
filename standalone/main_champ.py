...
def main() -> None:
    ...
    # 300s stagnation window — demo3 has long stretches where CFPA2 navigates
    # to a distant frontier without incrementing coverage. 90s fired too early.
    metrics = ExplorationMetricsLogger(
        ns,
        coverage_stagnant_window_sec=300.0,
        consec_no_reachable_threshold=5,
        run_name="cfpa2",
    )
    ...
    _last_failed_goal: list = [None]       # (gx, gy) of last blacklisted goal
    _last_fail_log_t: list = [-1e9]
    REPLAN_PERIOD_SEC = 1.0
    PATH_CHECK_SAMPLE = 3                  # every Nth waypoint for re-validation
    UNREACHABLE_FAIL_LIMIT = 1             # immediate blacklist on first A* failure
                                           # (CFPA2 BFS is inflation-blind so many goals
                                           # look reachable to it but A* with inflation
                                           # can't plan to them — no point retrying)
    FAIL_LOG_PERIOD_SEC = 5.0              # de-spam "no path" warnings
    FAILED_GOAL_RADIUS = 1.5              # new goal within this radius of a failed goal keeps fail count
    ...
    def _plan_and_apply(gx: float, gy: float, reason: str) -> bool:
        ...
        if path:
            ...
            return True
        _fail_count[0] += 1
        # Record this as a failed region immediately so subsequent CFPA2
        # re-publishes near the same frontier don't reset the counter.
        _last_failed_goal[0] = (gx, gy)
        now = env.time
        ...
        if _fail_count[0] >= UNREACHABLE_FAIL_LIMIT and _active_goal[0] is not None:
            log.warning(f"[CHAMP] goal ({gx:.2f},{gy:.2f}) unreachable — blacklisting")
    ...
    def _on_goal(msg) -> None:
        ...
        _goal_seq[0] += 1
        # Don't reset fail count if new goal is near the same region that
        # just failed — CFPA2 re-publishes slightly varied coords for the
        # same frontier, which previously reset the counter and prevented
        # blacklisting.
        last_fail = _last_failed_goal[0]
        if last_fail is None or math.hypot(gx - last_fail[0], gy - last_fail[1]) >= FAILED_GOAL_RADIUS:
            _fail_count[0] = 0
    ...
    import os as _os
    _report_root = Path(_os.environ.get("STANDALONE_RESULTS_DIR", "/tmp"))
    _report_root.mkdir(parents=True, exist_ok=True)
    report_path = _report_root / f"cfpa2_report_{_time.strftime('%Y%m%d_%H%M%S')}.json"
