...
class VLMExplorer:
    ...
    def __init__(
        ...
        *,
        ...
        self._model = model or (
            "gemini-2.0-flash" if self._provider == "google"
    ...
    def _build_scene(self) -> Tuple[dict, list]:
        ...
        if bfs_candidates and not frontiers:
            # A* rejected every BFS candidate — robot is either wedged against
            # a wall or the remaining frontiers are all in inflation-blocked zones.
            # Do NOT surface raw unvalidated candidates: the VLM would burn
            # validate_path calls on all of them, get reachable=False every time,
            # and exhaust the per-minute API quota (Groq TPM, Gemini RPM) in a
            # single cycle. Treat as no_frontiers — the run is effectively done.
            print(f"[VLM] A* rejected all {len(bfs_candidates)} BFS candidates "
                  f"— treating as no_frontiers (robot wedged or map complete).")
        ...
        def _worker() -> None:
            ...
            try:
                if vlm_backend.resolve_provider(provider) == "mock":
                    # Bypass LangGraph — use deterministic frontier pick.
                    raw = vlm_backend._mock_pick_goal(scene)
                    ...
                    decision = run_agent(
                        ...
                        temperature=0.1,
                        max_tokens=2048,
                        # Frontiers are pre-validated by A* before reaching the VLM,
                        # so validate_path is no longer a required step. The only
                        # tool call is log_artifact (if camera shows something).
                        # 1 iteration = 1 optional tool round + final JSON.
                        # This halves API usage vs the old validate_path-mandatory flow.
                        max_iterations=1,
                    )
                    err = None
            except Exception as exc:
